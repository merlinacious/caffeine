...
$content="ERR02: Unable to connect to database for feed";
$dbh = DBI->connect('DBI:mysql:kathabimb1:mysql.kathabimb.com', 'kathabimb', 'vesit!!roxx') || `echo "$content" | /usr/bin/mail -s "Feed Error for run on $d" "$admin" `;
